...
class colorRamp(NodeBase):
    def __init__(self, name):
        ...
        if self.input is not None: self.input.enableOptions(PinOptions.AlwaysPushDirty)
        self.output = self.createOutputPin('result', 'FloatPin', structure=StructureType.Array)
        if self.output is not None: self.output.enableOptions(PinOptions.AlwaysPushDirty)
