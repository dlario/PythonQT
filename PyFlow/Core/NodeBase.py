...
class NodeBase(INode):
    ...
    def createInputPin(self, pinName, dataType, defaultValue=None, foo=None, structure=StructureType.Single, constraint=None, structConstraint=None, supportedPinDataTypes=[], group="", **kwargs):
        """Creates input pin
        ...
        p = main.instance.packageRegister.CreateRawPin(pinName, self, dataType, PinDirection.Input)
        if p is not None:
    ...
    def createOutputPin(self, pinName, dataType, defaultValue=None, structure=StructureType.Single, constraint=None, structConstraint=None, supportedPinDataTypes=[], group="", **kwargs):
        """Creates output pin
        ...
        pinName = self.getUniqPinName(pinName)
        main = kwargs.get('main', False)
        p = main.instance.packageRegister.CreateRawPin(pinName, self, dataType, PinDirection.Output)
        p.structureType = structure
        p.group = group
    ...
    @staticmethod
    def initializeFromFunction(foo, **kwargs):
        ...
        if returnType is not None:
            p = raw_inst.createOutputPin('out', returnType, returnDefaultValue, supportedPinDataTypes=retAnyOpts, constraint=retConstraint, structConstraint=retStructConstraint,  **kwargs)
            if p is not None:
        ...
        for index in range(len(fooArgNames)):
            ...
            if str("Reference") == pinDescriptionTuple[0]:
                ...
                outRef = raw_inst.createOutputPin(argName, pinDataType, supportedPinDataTypes=anyOpts, constraint=constraint, structConstraint=structConstraint, **kwargs)
                ...
                inp = raw_inst.createInputPin(argName, pinDataType, supportedPinDataTypes=anyOpts, constraint=constraint, structConstraint=structConstraint, **kwargs)
                if inp is not None:
