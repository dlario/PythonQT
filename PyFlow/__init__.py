...
def loadPackages(Packages):
    __PACKAGES = Packages.PACKAGES
    __PACKAGE_PATHS = Packages.PACKAGE_PATHS
    __HASHABLE_TYPES = Packages.HASHABLE_TYPES
...
def GET_PACKAGE_CHECKED(package_name, **kwargs):
    main = kwargs.get("main", None)
    __PACKAGES = main.instance.packageRegister.PACKAGES
...
def CreateRawPin(name, owningNode, dataType, direction, **kwargs):
    pinClass = findPinClassByType(dataType)
    if pinClass is None:
        return None
    inst = pinClass(name, owningNode, direction, **kwargs)
    return inst

def getRawNodeInstance(nodeClassName, packageName=None, libName=None, **kwargs):
    from PyFlow.Core.NodeBase import NodeBase
    package = GET_PACKAGE_CHECKED(packageName, **kwargs)
