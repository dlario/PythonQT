...
from PackageManager.Packages.Template.Database.dbTemplate import *
# endregion

# region Default Settings]
from PackageManager.ConfigManager import ConfigManager
from PackageManager.Packages.Template.Database import DefaultData
...
class MDIMain(QMainWindow):
    ...
    def initDatabase(self):
        ...
        self.sqlRegister.registarInstance("TemplateLocation", "Template", self, self)
        self.sqlRegister.sqlDict["TemplateLocation"].createSession(base, name, dbtype, login, password, location, port,
                                                                   schema, key, sshTunnel)
        # This is the table coordinator
        self.session = self.sqlRegister.sqlDict["TemplateLocation"].self.session
        self.base = self.sqlRegister.sqlDict["TemplateLocation"].base
        self.sessions = {}

        self.setSessions(self.CurrentDatabase)
        self.sessionLocations = {}
        self.sessionLocations["Master"] = ["Location1", "TemplateLocation"]
        self.sessionLocations["Template"] = ["Location1", "TemplateLocation"]
