...
from PackageManager.Packages.ProgramBase.Database.dbFormBuilder import *
...
class FormBuilder(QMdiSubWindow):
    ...
    def dictFormImport(self, formfileName):
        ...
        TableImport = []
        TableImport.append("from PackageManager.Database import DatabaseTools")
        TableImport.append("from PackageManager.Packages.ProgramBase.Database.dbBase import *")
        TableImport.append("from PackageManager.Database.dbMasterTables import *")
        TableImport.append("from PackageManager.Packages.ProgramBase.Database.dbAllLists import *")
        TableImport.append("from PackageManager.Packages.ProgramBase.Database.dbAllTables import *")
    ...
    def on_cmdCreateFormBaseClass_clicked(self):
        ...
        copywritepath = os.path.join(basePackagePath, "Scripts/gnu.txt")
    ...
    def on_tvFormItems_load(self):
        ...
        if False:
            ...
            querytreepath = sessions["PackageManager"].query(FormDesignTree).filter_by(PrimaryGroup_id=1, SecondaryGroup_id=876, ParentTree_id="(0)").first()
            ...
            self.tvFormItemsmodel = TreeViewAlchemy2.linkedtreemodel(self.MasterData, treedicts["FormBuilder"], header="Dave Rocks")
            mrilist = []
            querytreepath = sessions["PackageManager"].query(FormDesignTree).filter_by(PrimaryGroup_id=1, ParentTree_id="(0)").all()

            for records in querytreepath:
                SecondaryGroup_id = records.SecondaryGroup_id
                PrimaryGroup_id = records.PrimaryGroup_id
                modelrootitem = sessions["PackageManager"].query(FormDesignTree).filter_by(
                    ParentTree_id="(0)") \
            ...
            if mrilist != []:
                self.tvFormItemsmodel.setupModelData(sessions["PackageManager"], FormDesignTree, mrilist)
    ...
    def createTreeFromData(self):
        #Root Record Group
        sessions["PackageManager"].begin()
        treerecords = sessions["PackageManager"].query(FormDesignTree)
        treerecords.delete(synchronize_session=False)
        sessions["PackageManager"].commit()
        ...
        #Build The Tree
        queryRecordGroup = sessions["PackageManager"].query(RecordGroup).order_by(RecordGroup.Title).all()
        ...
        for record in queryRecordGroup:
            ...
            queryRecordGroup = sessions["PackageManager"].query(FormItemData.id, FormItemData.ItemName, WidgetType.NameText) \
