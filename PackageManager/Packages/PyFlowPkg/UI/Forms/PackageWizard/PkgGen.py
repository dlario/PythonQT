...
from PackageManager.Packages.ProgramBase.UI import Wizards
...
def generatePackageInit(packageName,
    ...
    result += "from PackageManager.UI.UIInterfaces import IPackage\n\n"

    if bIncludePin:
        result += "# Pins\n"
        result += "from PackageManager.Packages.{0}.Pins.DemoPin import DemoPin\n\n".format(packageName)

    if bIncludeFooLib:
        result += "# Function based nodes\n"
        result += "from PackageManager.Packages.{0}.FunctionLibraries.DemoLib import DemoLib\n\n".format(packageName)

    if bIncludeClassNode:
        result += "# Class based nodes\n"
        result += "from PackageManager.Packages.{0}.Nodes.DemoNode import DemoNode\n\n".format(packageName)

    if bIncludeTool:
        result += "# Tools\n"
        result += "from PackageManager.Packages.{0}.Tools.DemoShelfTool import DemoShelfTool\n".format(packageName)
        result += "from PackageManager.Packages.{0}.Tools.DemoDockTool import DemoDockTool\n\n".format(packageName)

    if bIncludeExporter:
        result += "# Exporters\n"
        result += "from PackageManager.Packages.{0}.Exporters.DemoExporter import DemoExporter\n\n".format(packageName)

    result += "# Factories\n"
    if bIncludeUIPinFactory:
        result += "from PackageManager.Packages.{0}.Factories.UIPinFactory import createUIPin\n".format(packageName)

    if bIncludeUINodeFactory:
        result += "from PackageManager.Packages.{0}.Factories.UINodeFactory import createUINode\n".format(packageName)

    if bIncludePinInputWidgetFactory:
        result += "from PackageManager.Packages.{0}.Factories.PinInputWidgetFactory import getInputWidget\n".format(packageName)

    if bIncludePrefsWindget:
        result += "# Prefs widgets\n"
        result += "from PackageManager.Packages.{0}.PrefsWidgets.DemoPrefs import DemoPrefs\n".format(packageName)
...
def generatePackage(packageName,
    ...
    moduleInitFilePath = os.path.join(newPackagePath, "../../../../ProgramBase/UI/Forms/__init__.py")
