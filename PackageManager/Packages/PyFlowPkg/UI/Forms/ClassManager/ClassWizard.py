...
#from PackageManager.Packages.ProgramBase.UI.Forms.PackageWizard.PkgGen import *
...
uiFile = os.path.join(path, 'ClassManager - Import.ui')
...
class ClassWizard(TemplateBaseClass):
    def __init__(self):
    ...
    def onChangeFilterValue(self, text):
        ...
        search = QRegularExpression(str(text),
    ...
    def loadPinData(self, data):
        ...
        if "ENABLED_OPTIONS" in data:
            ...
            for option in options:
                ...
                if "AllowMultipleConnections" in option:   #: This enables pin to allow more that one input connection. See :func:`~PackageManager.Core.Common.connectPins`
                    self.ui.chkAllowMultipleConnections.setChecked(True)
                if "ChangeTypeOnConnection" in option:  #: Used by :class:`~PackageManager.Packages.ProgramBase.Pins.AnyPin.AnyPin` to determine if it can change its data type on new connection.
                ...
                if "AllowAny" in option:  #: Special flag that allow a pin to be :class:`~PackageManager.Packages.ProgramBase.Pins.AnyPin.AnyPin`, which means non typed without been marked as error. By default a :py:class:`PackageManager.Packages.ProgramBase.Pins.AnyPin.AnyPin` need to be initialized with some data type, other defined pin. This flag overrides that. Used in lists and non typed nodes
                    self.ui.chkAllowAny.setChecked(True)
                if "DictElementSupported" in option:  #: Dicts are constructed with :class:`DictElement` objects. So dict pins will only allow other dicts until this flag enabled. Used in :class:`~PackageManager.Packages.ProgramBase.Nodes.makeDict` node
    ...
    def defineFiles(self, f, foldername):
        ...
        for file in os.listdir(filepath):
            if file[1] != "_":
                ...
                #from PackageManager.Packages.ProgramBase.Pins.AnyPin import AnyPin
                f.write("from PackageManager.Packages.%s.%s.%s import %s\n"% (selectedpackage, foldername, file2, file2))
        f.write("\n")
