...
from PySide6.QtWidgets import QAction, QTextBrowser
from PackageManager.UI.Commands.Command import DockTool
#from PackageManager.UI.Views.NodeBox import NodesBox
from PackageManager.UI.Utils.stylesheet import editableStyleSheet
#from PackageManager.Core.GraphManager import GraphManagerSingleton
from PackageManager.Core.Common import SingletonDecorator
...
class LoggerTool(DockTool):
    ...
    def anchorClickedMethod(self, url):
        ...
        if os.path.exists(url.url().split("::")[0]):
            ...
            man = self.ProgramManagerInstance.graphManager
            node = man.get().findNode(str(url.url()))
            if node:
                self.ProgramManagerInstance.getCanvas().clearSelection()
                node.getWrapper().setSelected(True)
                self.ProgramManagerInstance.getCanvas().frameSelectedNodes()
    ...
    def do(self):
        self.ProgramManagerInstance.invokeDockToolByName(self.packageName, "Logger")
