""" Source: http://code.activestate.com/recipes/528868/
...
v1.0.5.DJL.01 - Worked on error handlers for new python
...
class AccessDb:
    ...
    def pyodbc_Connect1(self):
        #DBfile1 = 'Y:\IMR\RAT\RAT - Database.mdb'
        #User = "Dlario"
        #Password = "heyhey"
        #MDW = "yoyo"
        #conn2 = ODBCConnection.AccessDb(DBfile1, User, Password, MDW)
        #mstable = ODBCConnection.Table
    ...
    def pyodbc_Connect(self):
        DBfile1 = 'Y:\IMR\RAT\RAT - Database.mdb'
        DBfile2 = 'Y:\IMR\RAT\RAT - Office.mdb'
        DBfile3 = 'Y:\IMR\RAT\RAT - Projects.mdb'
        #DBfile4 = 'C:\Dropbox (Personal)\David Lario - Office\Research and Development\Data Skavenger.mdb;'
        DBfile4 = 'C:\Database\Data_Skavenger.mdb;'
        #connection_string = r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ='+DBfile4
        #conn = pyodbc.connect(connection_string, autocommit = True)

        conn = pyodbc.connect('Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ='+DBfile4)

        cursor = conn.cursor()
        cursor.execute('select count (*) from lstMasterList')
        row = cursor.fetchone()
        totalrows = row[0]

        currenttablerow = 0
        self.DataViewModel = QStandardItemModel(totalrows, 3, self)

        SQL = 'SELECT * FROM lstMasterList;'
        for record in cursor.execute(SQL): # cursors are iterable
            self.DataViewModel.setItem(currenttablerow, 0, QStandardItem(str(record[0])))
            self.DataViewModel.setItem(currenttablerow, 1, QStandardItem(str(record[1])))
            self.DataViewModel.setItem(currenttablerow, 2, QStandardItem(str(record[2])))
            currenttablerow += 1

        self.DataViewModel.setHeaderData(0, Qt.Horizontal, 'lstMasterlistID', role=Qt.DisplayRole)
        self.DataViewModel.setHeaderData(1, Qt.Horizontal, 'List Type', role=Qt.DisplayRole)
        self.DataViewModel.setHeaderData(2, Qt.Horizontal, 'List Name', role=Qt.DisplayRole)

        self.ui.tableView.setModel(self.DataViewModel)
