...
from Temp.version import Version
#from PackageManager.Core.Register import findPinClassByType
...
def myListToStr(myList):
    ...
    if myList is not None:
        for num, name in enumerate(myList):
            strList += "{} ".format(name)  # append the tuple in "num:name" format with a " " delimiter
    return strList[:-1]  # remove the final space (unneeded)

def strToMyList(myStr, bchar = " "):
    """This method takes a string in the format "int:str int:str int:str..."
    and converts it to a list of (int, str) tuples"""
    if myStr is None: myStr = ""
    myList = []
    if len(myStr):
        for numStr in myStr.split(bchar):  # for each converted tuple
            value = numStr # NOTE: this will throw an error if numStr.isdigit() is False
            myList.append(value)
    return myList
...
def cycleCheck(src, dst):
    ...
    :type src: :class:`PackageManager.Core.PinBase`
    :param dst: hand side pin
    :type dst: :class:`PackageManager.Core.PinBase`
...
def arePinsConnected(src, dst):
    ...
    .. note:: Pins can be passed in any order if **src** pin is :py:class:`PackageManager.Core.Common.PinDirection`, they will be swapped

    :param src: left hand side pin
    :type src: :py:class:`PackageManager.Core.PinBase`
    :param dst: right hand side pin
    :type dst: :py:class:`PackageManager.Core.PinBase`
...
def getConnectedPins(pin):
    ...
    :type pin: :py:class:`PackageManager.Core.PinBase.PinBase`
    :returns: Set of connected pins
    :rtype: set(:py:class:`PackageManager.Core.PinBase.PinBase`)
    """
    result = set()
...
def pinAffects(lhs, rhs):
    ...
    :type lhs: :py:class:`PackageManager.Core.PinBase.PinBase`
    :param rhs: Second Pin to connect
    :type rhs: :py:class:`PackageManager.Core.PinBase.PinBase`
    """
...
def canConnectPins(src, dst):
    ...
    :type src: :py:class:`PackageManager.Core.PinBase.PinBase`
    :param dst: Destination pin to connect
    :type dst: :py:class:`PackageManager.Core.PinBase.PinBase`
...
def connectPins(src, dst):
    ...
    * Input value pins can have one output connection if :py:class:`PackageManager.Core.Common.PinOptions.AllowMultipleConnections` flag is disabled
...
def traverseConstrainedPins(startFrom, callback):
    ...
    Iterates over all constrained chained pins of type :class:`Any <PackageManager.Packages.ProgramBase.Pins.AnyPin.AnyPin>` and passes pin into callback function. Callback will be executed once for every pin

    :param startFrom: First pin to start Iteration
    :type startFrom: :class:`~PackageManager.Core.PinBase.PinBase`
    :param callback: Functor to execute in each iterated pin.
    :type callback: callback(:class:`~PackageManager.Core.PinBase.PinBase`)
    """
...
def push(start_from):
    ...
    :type start_from: :py:class:`~PackageManager.Core.PinBase.PinBase`
    """
...
def removeDigitsFromEndOfString(string):
    ...
    Similar to :func:`~PackageManager.Core.Common.extractDigitsFromEndOfString`, but removes digits in the end.
...
class DictElement(tuple):
    """PackageManager dict element class
...
class PinOptions(Flag):
    ...
    .. seealso:: :meth:`~PackageManager.Core.PinBase.PinBase.enableOptions` :meth:`~PackageManager.Core.PinBase.PinBase.disableOptions`
    """
    ...
    AllowMultipleConnections = auto()  #: This enables pin to allow more that one input connection. See :func:`~PackageManager.Core.Common.connectPins`

    ChangeTypeOnConnection = auto()  #: Used by :class:`~PackageManager.Packages.ProgramBase.Pins.AnyPin.AnyPin` to determine if it can change its data type on new connection.
    ...
    AllowAny = auto()  #: Special flag that allow a pin to be :class:`~PackageManager.Packages.ProgramBase.Pins.AnyPin.AnyPin`, which means non typed without been marked as error. By default a :py:class:`PackageManager.Packages.ProgramBase.Pins.AnyPin.AnyPin` need to be initialized with some data type, other defined pin. This flag overrides that. Used in lists and non typed nodes
    DictElementSupported = auto()  #: Dicts are constructed with :class:`DictElement` objects. So dict pins will only allow other dicts until this flag enabled. Used in :class:`~PackageManager.Packages.ProgramBase.Nodes.makeDict` node
...
def findStructFromValue(value):
    """Finds :class:`~PackageManager.Core.Common.StructureType` from value

    :param value: input value to find structure.
    :returns: Structure Type for input value
    :rtype: :class:`~PackageManager.Core.Common.StructureType`
    """
...
class PinSelectionGroup(IntEnum):
    """Used in :meth:`~PackageManager.Core.NodeBase.NodeBase.getPinSG` for optimization purposes
    """
