...
from PySide6.QtCore import QSortFilterProxyModel, Slot, QRegularExpression, QItemSelectionModel, Qt
...
from PackageManager.Core import ProgramSettings
...
class TreeBuilderEntity():
    ...
    def __init__(self, TD1, tablesettings, sessionlist, baselist, parent = None, widget = None):
...
#items
    ...
    def LoadTreeModel(self):
        ...
        self.treemodel = TreeViewAlchemy2.linkedtreemodel(self.baselist, self.sessionlist, self.treedict, header=self.header)
        mrilist = []
    ...
    def on_AddDescriptorTable_clicked(self):
        ...
        AddNewItem = frmListEditor.UpdateList(self, tableName, self.treedict["Session"], Reference, session)
    ...
    def on_cmdAddDescriptorItem_clicked(self):
        ...
        if popup == True:
            ...
            if descriptorcount == 1:
                ...
                self.listeditor = frmListEditor.UpdateList(self, bases, self.sessions, DescriptorMasterTable)
    ...
    def on_cmdAddItem_clicked(self):
        ...
        if itemcount == 1:
            ...
            table_id = 194
            self.listeditor = frmListEditor.UpdateList(self, bases, self.sessions, table_id)
    ...
    def on_cmdAddAdjacent_clicked(self):
        ...
        treedict = self.treedict
        maxPG_id = int(self.dialog.txtPrimaryGroupID.text())
    ...
    def addTreeTag(self, newtreeitemdata):
        ...
        taglistquery = self.tsession.query(TagList).filter_by(Tag_id=tagid).first()
        ...
        if taglistquery is None:
             ...
             taglistquery = TagList(Tag_id=tagid,
                                   NameText=tagName)
             self.tsession.add(taglistquery)
        ...
        if len(Taglist) != 1:
            ...
            for count, eachtagitem in enumerate(Taglist):
                TagListQuery = self.tsession.query(TagList).filter_by(Tag_id=eachtagitem).first()
    ...
    def AddEventItem(self, MasterTable_id, Item_id, ParentTree_id=None):
        ...
        MasterTableData = self.tsession.query(ItemMasterTable).filter_by(id=MasterTable_id).first()
        ...
        listquery = self.sessions[MasterTableData.ReferenceTablself.treedict["Session"]].query(TableClass.id,
            ...
            if record is not None:
             ...
             #self.dialog.txtTreePath_2.setText(str(record.TreePath))
                ...
                if itemquery is not None:
                    ...
                    conn = self.sessions[SessionName.NameText].connection()
    ...
    def on_cmbPrimaryGroup_2_currentIndexChanged(self, index):
        ...
        if self.dialog.rdoExternal.isChecked():
            ...
            if DestinationSession_id:
                ...
                conn = self.treedict["MasterSession"].connection()
                sessionbase1 = DatabaseTools.get_class_by_tablename(self.baselist["PackageManager"], "lsttsession")
                self.selectedTableModel = QStandardItemModel(0, 2)
    ...
    def on_cmdCreateBranch_2_clicked(self):
        ...
        if destinationtable:
            itemcount = self.sessions[destinationSessionName.NameText].query(destinationtable).filter_by(
            ...
            TreeViewAlchemy2.clonebranch(sessions[sourceSessionName.NameText], sourcetable, OTree_id,
                                         self.sessions[destinationSessionName.NameText], destinationtable, "(0)",
                                         DPG_id=exportprimarykey, DSG_id=Secondary_id + 1)

            sourceparentitem = self.sessions[sourceSessionName.NameText].query(sourcetable).filter_by(
                Tree_id=OTree_id).first()

            TagList = sourceparentitem.Tags.split(" ")[-1]

            NameCount = self.sessions[destinationSessionName.NameText].query(destinationtable).filter_by(
    ...
    def on_cmdCreateFullBranch_clicked(self):
        ...
        if destinationtable:
            itemcount = self.sessions[destinationSessionName.NameText].query(destinationtable).filter_by(PrimaryGroup_id=exportprimarykey, ParentTree_id="(0)").count()
            ...
            sourceparentitem = self.sessions[sourceSessionName.NameText].query(sourcetable).filter_by(Tree_id=OTree_id).first()

            TreeViewAlchemy2.clonefullbranch(self.treedict, bases, self.sessions, self.sessions[sourceSessionName.NameText], sourcetable, OTree_id, sourceparentitem.PrimaryGroup_id, sourceparentitem.SecondaryGroup_id, self.sessions[destinationSessionName.NameText], destinationtable, "(0)", exportprimarykey, Secondary_id + 1)

            TagList = sourceparentitem.Tags.split(" ")[-1]

            NameCount = self.sessions[destinationSessionName.NameText].query(destinationtable).filter_by(PrimaryGroup_id=exportprimarykey, ParentTree_id="(0)", Tags=TagList).count()
    ...
    def on_cmbTreeSetting_currentIndexChanged(self, index):
        ...
        if itemquery is not None:
            ...
            if itemquery.ReferenceTablself.treedict["Session"] is not None:
                conn = self.sessions[itemquery.ReferenceTablself.treedict["Session"]].connection()
