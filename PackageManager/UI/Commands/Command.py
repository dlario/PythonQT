...
class CommandBase(object):
    ...
    def __init__(self):
        ...
        self.uid = uuid.uuid4()
        self.ProgramManagerInstance = None
        self.PackageInstance = None
    ...
    def setAppInstance(self, ProgramManagerInstance):
        if self.ProgramManagerInstance is None:
            self.ProgramManagerInstance = ProgramManagerInstance

    def setPackageInstance(self, PackageInstance):
        if self.PackageInstance is None:
...
class DockTool(QtWidgets.QDockWidget, CommandBase):
    ...
    def __init__(self):
        ...
        #self.setFeatures(QtGui.QDockWidget.DockWidgetFeatures(QtGui.QDockWidget.AllDockWidgetFeatures))
        '''self.setFeatures(QtWidgets.QDockWidget.DockWidgetFeatures(
    ...
    def closeEvent(self, event):
        self.onDestroy()
        #self.parent().unregisterCommandInstance(self)
        event.accept()
...
class DockTitleBar(QtWidgets.QWidget):
    def __init__(self, dockWidget, renamable=False):
        ...
        self.buttonsLay.setContentsMargins(0, 0, 0, 0)
    ...
    def onFeaturesChanged(self, features):
        if not features & QtWidgets.QDockWidget.DockWidgetVerticalTitleBar:
            ...
            self.dockButton.setVisible(True)
        else:
...
class DockToolBar(QtWidgets.QDockWidget, CommandBase):
    """Base class for dock tools
    """
    def __init__(self):
        CommandBase.__init__(self)
    ...
    def restoreState(self, settings):
        super(DockToolBar, self).restoreState(settings)
        self.setObjectName(self.uniqueName())
    ...
    def addButton(self, button):
        #This may have more function
        self.titleBarWidget().addButton(button)

class DockToolBarTitle(QtWidgets.QWidget):
    def __init__(self, dockWidget, renamable=False):
        super(DockToolBarTitle, self).__init__(dockWidget)
        self.renamable = renamable
        self.setLayout(QtWidgets.QHBoxLayout())
        self.layout().setContentsMargins(0, 0, 0, 1)
        self.buttonsLay = QtWidgets.QHBoxLayout()
    ...
    def eventFilter(self, source, event):
        if event.type() == QtCore.QEvent.WindowTitleChange:
            self.setTitle(source.windowTitle())
        return super(DockToolBarTitle, self).eventFilter(source, event)
    ...
    def update(self, *args, **kwargs):
        self.ChangeFloatingStyle(self.parent().isFloating())
        super(DockToolBarTitle, self).update(*args, **kwargs)
    ...
    def mouseDoubleClickEvent(self, event):
        if event.pos().x() <= self.titleLabel.width() and self.renamable:
            ...
            # this keeps the normal double-click behaviour
            super(DockToolBarTitle, self).mouseDoubleClickEvent(event)
