...
from PackageManager.UI.Canvas.UICommon import SessionDescriptor
from PackageManager.UI.Utils.stylesheet import editableStyleSheet, Colors
from PackageManager.Core.Common import *
from Temp import structs
...
class slider(QtWidgets.QSlider):
    ...
    def __init__(self, parent=None, draggerSteps=INT_SLIDER_DRAG_STEPS, sliderRange=[-100, 100], *args, **kwargs):
        ...
        self.MidButton = QtCore.Qt.MiddleButton
        self.draggers = None
        if SessionDescriptor().software == "maya":
            self.LeftButton = QtCore.Qt.MiddleButton
...
class pyf_timeline(QtWidgets.QSlider):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == QtCore.Qt.LeftButton and event.modifiers() != QtCore.Qt.AltModifier:
            butts = QtCore.Qt.MouseButtons(QtCore.Qt.MiddleButton)
            nevent = QtGui.QMouseEvent(event.type(), QtCore.QPointF(event.pos()), QtCore.QPointF(
                event.globalPos()), QtCore.Qt.MiddleButton, butts, event.modifiers())
    ...
    def mouseMoveEvent(self, event):
        if event.modifiers() == QtCore.Qt.AltModifier:
            if event.buttons() in [QtCore.Qt.MiddleButton, QtCore.Qt.LeftButton]:
...
class uiTick(QtWidgets.QGraphicsWidget):
    ...
    Holds a :obj:`PackageManager.Core.structs.Tick` inside with U,V attributes and expand it to use colors in V instead of floats for use in gradient sliders """
    def __init__(self, raw_tick, parent=None):
        """
        :param raw_tick: Input Core Tick
        :type raw_tick: :obj:`PackageManager.Core.structs.Tick`
...
class pyf_RampSpline(QtWidgets.QGraphicsView):
    """Ui Ramp/Curve Editor that encapsulates a :obj:`PackageManager.Core.structs.splineRamp` to edit it
    ...
    def __init__(self, raw_ramp, parent=None, bezier=False):
        ...
        :type raw_ramp: :obj:`PackageManager.Core.structs.splineRamp`
        :param parent: Parent QWidget
        :type parent: QtWidgets.QWidget, optional
    ...
    def updateFromRaw(self):
        """Updates Ui representation of the internal :obj:`PackageManager.Core.structs.splineRamp`
        """
        try:
    ...
    def addItem(self, u=0, v=0, raw_item=None):
        ...
        :param raw_item: Existing :obj:`PackageManager.Core.structs.Tick` to link with, if none, one new created , defaults to None
        :type raw_item: :obj:`PackageManager.Core.structs.Tick`, optional
        """
        if raw_item is None:
