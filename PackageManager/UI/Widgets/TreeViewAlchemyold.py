...
from lario import     DatabaseTools

import os
from lario import ProgramSettings
from PyQt5.QtWidgets import *

from PyQt5.QtCore import *
from PyQt5.QtGui import *

from dbEquipment import *
...
class sqlStandardItemModel(QStandardItemModel):
    ...
    def setupModelData2(self):
        ...
        records = self.session.query(self.TreeItemClass)
        recordrow = 0
...
class treeModel(QAbstractItemModel):
    def __init__(self, inParent=None, session=None, header="", groupid=0, tableclass=None, **kwargs):
 ...
 \
    ...
    def addTreeItem(self, treeItemDict):
        ...
        if CreateUpdateAttribute is None:
            CreateUpdateAttribute = TreeItems(TreeGroup_id=int(treeItemDict["TreeGroup_id"]),
                                              ...
                                              FlattenedOrder=treeItemDict["FlattenedOrder"],
                                              ItemLevel=int(treeItemDict["ItemLevel"]),
                                              ForeColor=treeItemDict["ForeColor"],
                                              Expanded=treeItemDict["Expanded"],
        ...
        treeItemDict["Tree_id"] = CreateUpdateAttribute.Tree_id
        self.session.commit()
        # print(treeItemDict)
        ...
        if treeItemDict['ItemLevel'] == 0:
            ...
            self.parents[CreateUpdateAttribute.id] = newparent
        else:
            ...
            self.parents[CreateUpdateAttribute.id] = newItem
    ...
    def setupModelData(self):
        if self.TreeItemClass != None:

            records = self.session.query(self.TreeItemClass) \
                .filter(self.TreeItemClass.TreeGroup_id == self.TreeGroup_id) \
                .order_by(TreeItems.ItemLevel, TreeItems.FlattenedOrder).all()

            for treeRecords in records:
                Display = []

                Display.append(treeRecords.DisplayName)
                print(treeRecords.id, treeRecords.DisplayName)
                treeItemDict = self.currentTreeItem(treeRecords.id)
                print(treeItemDict)
                if treeItemDict != {}:
                    TIC = treeItem_class(treeItemDict)
                    ...
                    if treeItemDict['ItemLevel'] == 0:
                        # Root Level Entries
                        newparent = TreeItem(TIC, treeRecords.Header, self.rootItem)
                        self.rootItem.appendChild(newparent)
                        # Create the Dictonary Linking Values
                        self.parents[treeItemDict['Tree_id']] = newparent
                    else:
                        parentItem = self.parents[treeItemDict['ParentTree_id']]
                        newItem = TreeItem(TIC, treeRecords.Header, parentItem)
                        parentItem.appendChild(newItem)
                        self.parents[treeItemDict['Tree_id']] = newItem
