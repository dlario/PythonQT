...
#from PackageManager.UI.Widgets.BlueprintCanvas import BlueprintCanvasWidget
from PackageManager.UI.Tool.Tool import ShelfTool, DockTool
from PackageManager.UI.EditorHistory import EditorHistory
from PackageManager.UI.Tool import GET_TOOLS
from PackageManager.UI.Utils.stylesheet import editableStyleSheet
from PackageManager.UI.ContextMenuGenerator import ContextMenuGenerator
...
from PackageManager import INITIALIZE
...
def winTitle():
    return "PackageManager v{0}".format(currentVersion().__str__())
...
class PackageManager(QMainWindow):
    ...
    def populateMenu(self):
        ...
        helpMenu.addAction("Homepage").triggered.connect(lambda _=False, url="https://enigma-design.ca/": QtGui.QDesktopServices.openUrl(url))
        helpMenu.addAction("Docs").triggered.connect(lambda _=False, url="https://PackageManager.readthedocs.io/en/latest/": QtGui.QDesktopServices.openUrl(url))
    ...
    def closeEvent(self, event):
        ...
        PackageManager.appInstance = None
    ...
    def instance(parent=None, software=""):
        ...
        instance = PackageManager(parent)
        ...
        for packageName, registeredToolSet in GET_TOOLS().items():
            for ToolClass in registeredToolSet:
                if issubclass(ToolClass, ShelfTool):
                    ...
                    ToolInstance.setAppInstance(instance.main)
                    ToolInstance.setPackageInstance(instance)
        ...
        PackageManager.appInstance = instance
        EditorHistory().saveState("New file")
