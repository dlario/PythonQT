...
class TableView(QTableView, ViewBase):
    """
    ...
    def __init__(self, parent=None, extensionId=None):
        ...
        self.del_key = QShortcut(self)
        self.del_key.setKey(Qt.CTRL + Qt.Key_Delete)
        self.del_key.setContext(Qt.WidgetWithChildrenShortcut)
    ...
    def moveCursor(self, cursorAction, modifiers):
        if self.use_edit_tab_semantics == True and cursorAction in [QAbstractItemView.MoveNext,  QtGui.QAbstractItemView.MovePrevious]:
            ...
            if model.columnCount(index.parent()) > 0 and model.rowCount(index.parent()) > 0:
                ...
                if cursorAction == QtGui.QAbstractItemView.MoveNext:
                    while iterations < cell_max:
                        ...
                        if (model.flags(candidate) & Qt.ItemIsEditable) == Qt.ItemIsEditable:
                            return candidate
                        index = candidate
    ...
    def postReset(self):
        if hasattr(self, "preserved_id"):
            ...
            for i in range(self.model().rowCount(QModelIndex())):
                if oc(self.model().index(i, 0, QModelIndex()).internalPointer()) == self.preserved_id:
                    self.setCurrentIndex(self.model().index(i, 0, QModelIndex()))
                    break
