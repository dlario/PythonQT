...
class BoundCommandMenuItem(object):
    ...
    def __init__(self, cmd, action, objectConverter, binding):
        ...
        if isinstance(self.action, QPushButton):
            self.action.clicked.connect(self.triggeredProxy)
        elif isinstance(self.action, QAction):
...
class BoundCommandMenu(QtCore.QObject):
    ...
    def resetActions(self):
        ...
        if actionParent is None and isinstance(self.target, QWidget):
        ...
        for cmd in self.menu.commands:
            ...
            if self.buttonBox is not None:
                #TODO:  map Command.type to QDialogButtonBox.*Role items
                button = self.buttonBox.addButton(cmd.descr, QDialogButtonBox.ActionRole)
