...
from sqlalchemy.orm import attributes, MapperEvents
...
class ModelChangeExtension:
    ...
    def __call__(self, *args):
        inst, attr_name, old_value, new_value = args

        if self.on_change is not None and new_value != old_value:
            self.on_change(inst, attr_name, old_value, new_value)

    def __getstate__(self):
        return {'on_change': self.on_change}

    def __setstate__(self, state):
        self.on_change = state['on_change']
...
# TODO: fix this
'''@event.listens_for(attributes.Event, 'set')
def on_set(target, value, oldvalue, initiator):
    if initiator:
        extensions = initiator.extensions
        for ext in extensions:
            if isinstance(ext, ModelChangeExtension):
                ext(target, initiator.key, oldvalue, value)'''
...
class EventExtension(MapperEvents):
    """
