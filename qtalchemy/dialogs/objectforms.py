...
from sqlalchemy.orm import mapper, create_session, object_session, Query
import traceback
import sys
...
def MessageButtonsToQt(flags):
    ...
    for x in "Ok Cancel Yes No".split(' '):
        if flags & getattr(Message, x):
            result |= getattr(QMessageBox, x)
    return result

def QtToMessageButtons(flags):
    result = 0
    for x in "Ok Cancel Yes No".split(' '):
        if flags & getattr(QMessageBox, x):
...
def messaged_commit(session, parent):
    ...
    try:
        ...
        exception_message_box(e,"There was an error saving the data.",parent=parent,icon=QMessageBox.Warning)

class MapperDialog(QDialog, MapperMixin):
...
class BoundDialog(MapperDialog):
    ...
    def preCommandSave(self, event):
        ...
        try:
            ...
            exception_message_box(e,"There was an error saving data.",icon=QMessageBox.Warning,parent=self)
            event.abort()
...
def BoundDialogObject(parent,row):
    ...
    main = QVBoxLayout()
    b.setLayout(main)
    grid = LayoutLayout(main,QFormLayout())
    ...
    buttonbox = LayoutWidget(main,QDialogButtonBox())
