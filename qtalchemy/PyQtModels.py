...
class AlchemyModelDelegate(QStyledItemDelegate):
...
def AttrWidgetFactory(userattr, parent=None, index=None):
    ...
    if myAttrType == datetime.date:
        ...
        return QCheckBox(ClassAttributeLabel(userattr), parent)
    elif myAttrType == int:
        w = QLineEdit(parent)
        ...
        w = QLineEdit(parent)
        w.setValidator(QDoubleValidator(parent))
        return w
    else:
...
def ClassTableModel(cls, attributes, readonly=True, fixed_rows=False, row_manager=None, objectConverter=None,
    ...
    Example:
        ...
        ''
        >>> model.setData(index,"sam",Qt.EditRole)
        True
        >>> p.nicks
        [Nick:  sam]
        >>> index = model.index(1,0,None)
        >>> model.setData(index,"jack",Qt.EditRole)
        True
        >>> p.nicks
        [Nick:  sam, Nick:  jack]
    """
    ...
    def makeModelColumn(cls, attr):
        ...
        return ModelColumn(attr, ClassAttributeType(ua), ClassAttributeLabel(ua), readonly=attrReadonly(cls, attr),
                           alignment=attrAlignment(cls, attr))

    cols = [makeModelColumn(cls, a) for a in attributes]
    return PBTableModel(cols, cls=cls, readonly=readonly, fixed_rows=fixed_rows, row_manager=row_manager,
                        objectConverter=objectConverter, tree_structure=tree_structure)
...
def QueryTableModel(q, cols=None, include_uuid=False, ssrc=None, objectConverter=None, tree_structure=None):
    ...
    for col in sq.columns:
        if col.name.startswith("_hidden_"):
            ...
            assert match, print(f"In {col.name}, the sqlalchemy type {col.type} was not known for mapping to a python type.")
...
class PBTableModel(QAbstractItemModel):
    """
    ...
    def __init__(self, columns, cls=None, rows=None, query=None, ssrc=None, parent=None, readonly=True,
                 row_manager=None, fixed_rows=False, objectConverter=None, tree_structure=None):
        ...
        self.setRoleNames = {Qt.UserRole + i: self.columns[i].attr for i in range(len(columns))}
    ...
    def data(self, index, role):
        ...
        if role == Qt.DisplayRole:
            ...
            if primary_yoke == 'bool':
                return Qt.Checked if primary_value else Qt.Unchecked
            ...
            return int({"left": Qt.AlignLeft, "hcenter": Qt.AlignHCenter, "right": Qt.AlignRight}[hori] | \
                       {"top": Qt.AlignTop, "vcenter": Qt.AlignVCenter, "bottom": Qt.AlignBottom}[vert])
            ...
            assert Qt.UserRole <= role < Qt.UserRole + len(self.columns)
            roles = self.roleNames()
            c = self.columns[role - Qt.UserRole]
            return toQType(primary_value, suggested=primary_type)
    ...
    def ensureNoFlipper(self, index):
        if not self.row_manager.fixed_rows and index.row() >= len(self.row_list):
            ...
            if len(self.flipper_rows) == 0:
                self.beginInsertRows(QModelIndex(), len(self.row_list), len(self.row_list))
                self.flipper_rows.append(self.row_manager.add_flipper_row())
                self.endInsertRows()
...
def modelMimeRectangle(model, selected):
    ...
    def row_data(full_cols, my_indexes):
        ...
        for x in full_cols:
            r = ""
            if x in my_cols:
                cellData = fromQType(model.data(my_cols[x], Qt.DisplayRole))
                if cellData is None and model.data(my_cols[x], Qt.CheckStateRole) is not None:
                    cellData = model.data(my_cols[x], Qt.CheckStateRole) == Qt.Checked
                r = str(cellData)
            result.append(r)
        return result
    ...
    if len(selected):
        ...
        textrows = ['\t'.join([model.headerData(x, Qt.Horizontal, Qt.DisplayRole) for x in cols])]
        for y in rows:
...
class WidgetAttributeMapper(object):
    ...
    def addBoundForm(self, layout, attrs):
        ...
        """
        grid = LayoutLayout(layout, QFormLayout())
    ...
    def labelAlignment(self, layout, labelPos):
        ...
        if style is not None and labelPos == "l":
            return Qt.Alignment(style.styleHint(QStyle.SH_FormLayoutLabelAlignment))
        return None
    ...
    def addBoundFieldGrid(self, grid, attr, row, column, rowSpan=1, columnSpan=1, labelPos="left"):
        ...
        if labelPos not in ['', None]:
            ...
            label_cell = {'l': (row, column), 'a': (row, column), 't': (row, column), 'r': (row, column + 1),
                          'b': (row + 1, column)}.get(label_key)
            widget_cell = {'l': (row, column + 1), 'a': (row + 1, column), 't': (row + 1, column), 'r': (row, column),
                           'b': (row, column)}.get(label_key)

            label = QLabel(attrLabel(self.cls, attr))
